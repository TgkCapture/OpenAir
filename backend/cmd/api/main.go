...
func main() {
	...
	if err := token.EnsureKeysExist(cfg.JWTPrivateKeyPath, cfg.JWTPublicKeyPath); err != nil {
		log.Fatalf("failed to ensure JWT keys: %v", err)
	}
	...
	srv := server.New(cfg, db, rdb, tokenManager)
